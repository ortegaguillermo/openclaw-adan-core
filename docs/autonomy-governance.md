...
- **Policy is ENABLED by default** with safe, conservative defaults.
- All high-risk actions are disabled by default (`allowAutoclosePR: false`, `allowAutomergePR: false`, `allowAutotagRelease: false`).

### Step 2: Add Policy Enforcement
- Update `WORKFLOW_AUTO.md` with authorization checks.
- **Behavior change is ACTIVE** as policy is enabled by default.
- Existing agents must have valid `adan.flags.json`; missing `autonomyPolicy` block assumes strict defaults.
